...
minmax x y z = maximum x y z - minimum x y z where {
    maximum x y z = max z (max x y);
    minimum x y z = min z (min x y)
}
...
lowerAndGreater n list = show n ++ " is greater than " ++ show (getGreaterCount n list 0) ++ " elements and lower than " ++ show (getLowerCount n list 0) ++ " elements" where {
    getGreaterCount num listOfNum count | null listOfNum = count 
                                    | num <= minimum listOfNum = count
                                    | num > minimum listOfNum = getGreaterCount num (tail (sort listOfNum)) (count + 1);
    getLowerCount num listOfNum count | null listOfNum = count 
                                    | num >= maximum listOfNum = count
                                    | num < maximum listOfNum = getLowerCount num (tail (reverse (sort listOfNum))) (count + 1)
}
